var Rollup = (function (exports) {
	...
	class CellManager {
	    ...
	    getCellHTML(cell) {
	        ...
	        var isBodyCell = !(isHeader || isFilter);
	    ...
	    applyFilter(filters) {
	        ...
	        this.datamanager.filterRows(filters)
	            .then(function (ref) {
	                ...
	                var rows = rowsToShow.map(function (rowIndex) { return this$1.datamanager.getRow(rowIndex); });
	                this$1.bodyRenderer.renderRows(rows);
	            });
	    }
	...
	class RowManager {
	    ...
	    hideRows(rowIndices) {
	        ...
	        var selector = rowIndices.map(this.selector).join(',');
	    ...
	    openSingleNode(rowIndex) {
	        var rowsToShow = this.datamanager.getImmediateChildren(rowIndex);
	    ...
	    closeSingleNode(rowIndex) {
	        ...
	        var children = this.datamanager.getImmediateChildren(rowIndex);
	        children.forEach(function (childIndex) {
	            var row = this$1.datamanager.getRow(childIndex);
	            if (row.meta.isLeaf) {
	                // close
	                this$1.hideRows(childIndex);
	                this$1.cellmanager.toggleTreeButton(childIndex, false);
	            } else {
	                this$1.closeSingleNode(childIndex);
	                this$1.hideRows(childIndex);
	            }
	        });
	...
	class BodyRenderer {
	    ...
	    renderRows(rows) {
	        ...
	        var config = {
	            ...
	            generate: function (index) {
	                ...
	                var rowHTML = this$1.rowmanager.getRowHTML(rows[index], rows[index].meta);
	    ...
	    getDataForClusterize(rows) {
	        ...
	        return rows.map(function (row) { return this$1.rowmanager.getRowHTML(row, row.meta); });
	    }

	    getBodyHTML(rows) {
	        var this$1 = this;

	        return ("\n            <div class=\"dt-body\">\n                <div>\n                    " + (rows.map(function (row) { return this$1.rowmanager.getRowHTML(row, row.meta); }).join('')) + "\n                </div>\n            </div>\n        ");
	    }
	...
	class Style {
	    ...
	    compensateScrollbarWidth() {
	        ...
	        requestAnimationFrame(function () {
	            ...
	            var lastCol = this$1.datamanager.getColumn(-1);
	            var width = lastCol.width - scrollbarWidth;
	            this$1.columnmanager.setColumnWidth(lastCol.colIndex, width);
	        });
	    }
	    ...
	    distributeRemainingWidth() {
	        ...
	        var headerWidth = $.style(this.header, 'width');
	    ...
	    setDefaultCellHeight() {
	        ...
	        var $firstCell = $('.dt-cell--header', this.instance.header);
	        if (!$firstCell) { return; }

	        var height = this.options.cellHeight || $.style($firstCell, 'height');
	        if (height) {
	    ...
	    setBodyStyle() {
	        ...
	        requestAnimationFrame(function () {
	            var width = $.style(this$1.header, 'width');

	            $.style(this$1.bodyScrollable, {
	                width: width + 'px'
	            });

	            $.style(this$1.bodyScrollable, {
	                marginTop: $.style(this$1.header, 'height') + 'px'
	            });
	        });
	    }
	        ...
	        contains: function contains(keyword, cells) {
	            return cells
	                .filter(function (cell) {
	                    var hay = String(cell.content || '').toLowerCase();
	...
	frappe.views.QueryReport = class QueryReport extends frappe.views.BaseList {
		...
		setup_events() {
			...
			frappe.realtime.on("report_generated", function (data) {
				if(data.report_name) {
					var alert_message = "Report " + (this$1.report_name) + " generated.\n\t\t\t\t\t<a target='_blank' href=\"#query-report/" + (this$1.report_name) + "\">View</a>";
			...
			this.export_dialog = frappe.prompt({
				...
				if (file_format === 'CSV') {
					...
					var args = {
						...
						filters: filters,
						visible_idx: this$1.datatable.datamanager.getFilteredRowIndices(),
					};
		...
		get_data_for_csv() {
			...
			var out = indices.map(function (i) { return this$1.datatable.datamanager.getRow(i).map(function (c) { return c.content; }); });
			return out.map(function (row) { return row.slice(1); });
		}
		...
		toggle_nothing_to_show(flag) {
			var message = __('Nothing to show');
